...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # ── Demo lenta postlarini o'chirish (eski seed data) ──────
        deleted, _ = Murojaat.objects.filter(telegram_user_id__in=[2000001, 2000002, 2000003]).delete()
        if deleted:
            self.stdout.write(self.style.SUCCESS(f"{deleted} ta demo lenta posti o'chirildi."))
